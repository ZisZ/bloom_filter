#ifndef PARTIAL_BLOOM_FILTER_H_
#define PARTIAL_BLOOM_FILTER_H_
...
class PartialBloomFilter : public BloomFilterBase<Hasher> {
  static const typename Hasher::ResultType InitialSeed = 0;
 public:
  PartialBloomFilter(uint64_t element_capacity, double false_positive_prob) {
    InitialClear();
    Initialize(element_capacity, false_positive_prob);
  }

  PartialBloomFilter(uint64_t bitmap_byte_size, uint32_t num_hashes) {
    InitialClear();
  ...
  bool Insert(const void *key, uint32_t len) {
    ...
    for (uint32_t i = 0; i < this->num_hash_functions_; ++i) {
      ...
      typename Hasher::ResultType hash_i = seed % this->num_bits_per_part_;
      uint64_t hash = hash_i + i * this->num_bits_per_part_;
 ...
 private:
  ...
  void UncheckedInitialize(uint64_t bitmap_byte_size, uint32_t num_hashes) {
    ...
    this->num_bits_per_part_ = static_cast<uint64>(this->num_bits_ / this->num_hash_functions_);
    ...
    LOG(INFO) << "Bloom filter bits wasting: " << this->num_bits_ % this->num_bits_per_part_;
  }

 private:
  PartialBloomFilter(const PartialBloomFilter&);
  PartialBloomFilter& operator=(const PartialBloomFilter&);
 ...
 private:
  uint64_t  num_bits_per_part_;
};

}  // namespace bloom

#endif  // PARTIAL_BLOOM_FILTER_H_
